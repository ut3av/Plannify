...
export default function App() {
  ...
  const [teachers, setTeachers] = useState([
    { name: "Asha Rao", free_periods: 1 },
    { name: "Ben Thomas", free_periods: 1 },
    { name: "Chen Lee", free_periods: 1 },
    { name: "Divya Shah", free_periods: 1 }
  ]);
  ...
  const [subjects, setSubjects] = useState([
    { code: "201", name: "Data Structures", teacher: "Asha Rao", section: "BCA-A", required_slots: 4 },
    { code: "202", name: "Operating System", teacher: "Ben Thomas", section: "BCA-A", required_slots: 3 },
    { code: "203", name: "Environmental Studies", teacher: "Chen Lee", section: "BCA-B", required_slots: 3 },
    { code: "204", name: "Basic Communication", teacher: "Divya Shah", section: undefined, required_slots: 2 },
    { code: "206", name: "Programming Lab in Data Structures", teacher: "Asha Rao", section: "BCA-A", is_lab: true, required_slots: 2 },
  ]);
  const [rooms, setRooms] = useState(["Room 101", "Room 102", "Lab 1"]);
  ...
  const rescheduleTimetable = async (request) => {
    ...
    try {
      ...
      setRescheduleNote(`${request.teacher} unavailable rule applied to ${blocked} slot(s).`);
      setActiveTab("timetable");
    } catch (apiError) {
      setError(getErrorMessage(apiError));
    } finally {
  ...
  const assignProxy = async (request) => {
    ...
    try {
      ...
      setResult(response.data);
      setRescheduleNote(response.data.reschedule_note?.message || "Proxies assigned.");
      setActiveTab("timetable");
    } catch (apiError) {
      setError(getErrorMessage(apiError));
    } finally {
    ...
    sectionsSet.forEach(section => {
      ...
      result.days.forEach(day => {
        ...
        for (let i = 1; i < timeSlotsRow.length; i++) {
           ...
           const secAssigned = assignments.find(a => a.section === section || (!a.section && section === "Default"));
           
           if (!secAssigned) {
             row.push("");
           } else {
             if (secAssigned.code || secAssigned.subject) {
                sectionSubjectsMap.set(secAssigned.code || secAssigned.subject, secAssigned);
             }
      ...
      worksheet['!merges'].push({ s: {r: subjHeaderRowIdx, c: 0}, e: {r: subjHeaderRowIdx, c: 1} });
      ...
      lunchColIdxList.forEach(colIdx => {
         ...
         worksheet['!merges'].push({ s: {r: startRow, c: colIdx}, e: {r: endRow, c: colIdx} });
      });
    });
    ...
    teachersSet.forEach(teacherName => {
      ...
      result.days.forEach(day => {
        ...
        for (let i = 1; i < timeSlotsRow.length; i++) {
           ...
           const slotName = result.time_slots[slotCounter];
           const assignments = result.timetable?.[day]?.[slotName] || [];
           const teacherAssigned = assignments.find(a => a.teacher === teacherName);
           
           if (!teacherAssigned) {
             row.push("");
           } else {
             const codeDisplay = teacherAssigned.code ? teacherAssigned.code : teacherAssigned.subject;
             row.push(`${codeDisplay} (${teacherAssigned.room}) [${teacherAssigned.section || 'Auto'}]`);
           }
           slotCounter++;
        }
        rows.push(row);
      ...
      let sheetName = teacherName.replace(/[^a-zA-Z0-9 ]/g, "").substring(0, 31);
      if (!sheetName) sheetName = "Teacher1";
      // ensure unique
      while (workbook.SheetNames.includes(sheetName)) sheetName = sheetName.substring(0, 28) + (Math.floor(Math.random() * 100));
      XLSX.utils.book_append_sheet(workbook, worksheet, sheetName);
    });
  ...
  const saveToDatabase = async () => {
    ...
    try {
      ...
      setRescheduleNote("Timetable saved to free SQLite database successfully!");
    } catch (e) {
  ...
  const totalSlots = useMemo(() => subjects.reduce((sum, s) => sum + s.required_slots, 0), [subjects]);
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "sections":
        return <SectionsSection sections={sections} rooms={rooms} onChange={setSections} />;
      case "subjects":
        return <SubjectsSection subjects={subjects} teachers={teachers} sections={sections} onChange={setSubjects} />;
      case "rooms":
        return <RoomsSection rooms={rooms} onChange={setRooms} />;
      case "slots":
        return <TimeSlotsSection timeSlots={timeSlots} onChange={setTimeSlots} />;
      case "timetable":
        return <TimetableGrid result={result} loading={loading} onExport={exportToExcel} onSaveDb={saveToDatabase} />;
      case "reschedule":
        return (
          <ReschedulePanel
            teachers={teachers}
      ...
      <div className="relative mx-auto flex w-full max-w-7xl flex-col gap-0 px-4 py-6 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-violet-500/20">
                <TabIcon icon="sparkles" className="w-6 h-6 text-slate-900 dark:text-white" />
              </div>
              <div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="hidden sm:flex items-center gap-4 mr-2">
                <div className="text-center">
                  ...
                  <p className="text-[10px] uppercase tracking-wider text-slate-500">Teachers</p>
                </div>
                <div className="w-px h-8 bg-slate-200 dark:bg-white/10" />
                <div className="text-center">
                  <p className="text-lg font-bold text-slate-900 dark:text-white">{subjects.length}</p>
                  <p className="text-[10px] uppercase tracking-wider text-slate-500">Subjects</p>
                </div>
                <div className="w-px h-8 bg-slate-200 dark:bg-white/10" />
                <div className="text-center">
                  <p className="text-lg font-bold text-slate-900 dark:text-white">{totalSlots}</p>
              ...
              <button
                ...
                className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] flex items-center justify-center text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
                title="Toggle Theme"
              >
                {isDark ? (
          ...
          {result && (
            <div className="mt-4 pt-4 border-t border-slate-200 dark:border-white/[0.06] flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2 rounded-lg bg-violet-500/10 border border-violet-500/20 px-2.5 py-1">
                ...
                <span className="text-xs font-bold text-violet-200">{result.objective_score}</span>
              </div>
              <div className="flex items-center gap-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1">
                ...
                <span className="text-xs font-bold text-emerald-200">{result.assignments?.length || 0}</span>
              </div>
            </div>
          )}
        </header>
        ...
        {rescheduleNote && (
          <div className="animate-slide-down flex items-start gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 backdrop-blur-sm px-5 py-4 text-sm text-emerald-200 mb-4">
            <svg className="w-4 h-4 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 11.08V12a10 10 0 11-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg>
