      ...
      <nav className="sticky top-0 z-50 glass-panel border-b border-[#E8DDD0] dark:border-white/10 px-4 sm:px-6 py-3.5 flex justify-between items-center bg-white/95 dark:bg-slate-950/90 backdrop-blur-2xl">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              <svg className="w-4 h-4 text-amber-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
          ...
          {onToggleTheme && (
            ...
            >
              {theme === "dark" ? (
                ...
                <svg className="w-4 h-4 text-slate-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
              )}
            </button>
          )}
        ...
        <div className="card p-4 flex flex-wrap items-center justify-between gap-4 text-xs mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2.5 text-slate-600 dark:text-slate-300">
            {(selectedTeacher?.employee_id || user?.user_metadata?.employee_id) && (
              <span className="font-mono text-[11px] bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-700 text-amber-800 dark:text-amber-300 flex items-center gap-1">
                <svg className="w-3 h-3 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>
                {selectedTeacher?.employee_id || user?.user_metadata?.employee_id}
              </span>
            )}
            {(selectedTeacher?.email || user?.email) && (
              <span className="bg-slate-100 dark:bg-slate-800/60 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-700/60 text-slate-600 dark:text-slate-300 flex items-center gap-1">
                <svg className="w-3 h-3 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                {selectedTeacher?.email || user?.email}
              </span>
            )}
