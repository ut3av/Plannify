...
export default function InstitutionalDashboard({
  ...
  return (
    <div className="space-y-6 animate-fade-in text-slate-900 dark:text-slate-100">
      ...
      <div className="card p-6 bg-gradient-to-br from-[#1C120B] via-[#2A160D] to-[#120A05] border border-[#3D2517] text-amber-100 relative overflow-hidden shadow-xl">
        {/* Ambient Glows */}
        <div className="absolute top-0 right-0 w-96 h-96 bg-amber-500/10 blur-3xl rounded-full pointer-events-none" />
        <div className="absolute bottom-0 left-1/3 w-80 h-80 bg-orange-600/10 blur-3xl rounded-full pointer-events-none" />
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 relative z-10">
          <div className="max-w-xl">
            ...
            <p className="text-xs text-[#D4C4B0] mt-2 leading-relaxed">
            ...
            <div className="flex items-center gap-3 mt-4">
              <button
                ...
                className="btn-primary text-xs py-2.5 px-4 font-bold gap-2 shadow-lg shadow-amber-900/30 flex items-center hover:scale-105 active:scale-95 transition-transform"
              >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
        {/* Card 1: Faculty */}
        <div className="card p-4 stat-card-elevate animate-slide-up-fade stagger-1 border-slate-800 bg-slate-900/90 relative overflow-hidden">
          ...
          <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden mt-2">
            <div className="h-full bar-fill-anim bg-amber-500" style={{ width: `${Math.min(activeFaculty * 8, 100)}%` }} />
          </div>
          <p className="text-[10px] text-slate-500 mt-1">Active Profiles</p>
        </div>

        {/* Card 2: Sections */}
        <div className="card p-4 stat-card-elevate animate-slide-up-fade stagger-2 border-slate-800 bg-slate-900/90 relative overflow-hidden">
          ...
          <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden mt-2">
            <div className="h-full bar-fill-anim bg-amber-400" style={{ width: `${Math.min(sectionsCount * 25, 100)}%` }} />
          </div>
          <p className="text-[10px] text-slate-500 mt-1">Active Classes</p>
        </div>

        {/* Card 3: Subjects */}
        <div className="card p-4 stat-card-elevate animate-slide-up-fade stagger-3 border-slate-800 bg-slate-900/90 relative overflow-hidden">
          ...
          <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden mt-2">
            <div className="h-full bar-fill-anim bg-orange-400" style={{ width: `${Math.min(subjectsCount * 12, 100)}%` }} />
          </div>
          <p className="text-[10px] text-slate-500 mt-1">Course Catalog</p>
        </div>

        {/* Card 4: Classrooms */}
        <div className="card p-4 stat-card-elevate animate-slide-up-fade stagger-4 border-slate-800 bg-slate-900/90 relative overflow-hidden">
          ...
          <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden mt-2">
            <div className="h-full bar-fill-anim bg-purple-400" style={{ width: `${Math.min(roomsCount * 20, 100)}%` }} />
          </div>
          <p className="text-[10px] text-slate-500 mt-1">Rooms & Labs</p>
        </div>

        {/* Card 5: Attendance Rate */}
        <div className="card p-4 stat-card-elevate animate-slide-up-fade stagger-5 border-slate-800 bg-slate-900/90 relative overflow-hidden">
          ...
          <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden mt-2">
            <div className="h-full bar-fill-anim bg-emerald-400" style={{ width: `${attendanceRate}%` }} />
          </div>
          <p className="text-[10px] text-emerald-500/80 mt-1">Today's Rate</p>
        </div>

        {/* Card 6: On Leave */}
        <div className="card p-4 stat-card-elevate animate-slide-up-fade stagger-6 border-slate-800 bg-slate-900/90 relative overflow-hidden">
          ...
          <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden mt-2">
            <div className="h-full bar-fill-anim bg-amber-300" style={{ width: `${Math.min(onLeaveToday * 30, 100)}%` }} />
          </div>
          <p className="text-[10px] text-slate-500 mt-1">Approved Leave</p>
        </div>

        {/* Card 7: Pending Leaves */}
        <div className="card p-4 stat-card-elevate animate-slide-up-fade stagger-7 border-slate-800 bg-slate-900/90 relative overflow-hidden">
          ...
          <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden mt-2">
            <div className="h-full bar-fill-anim bg-rose-400" style={{ width: `${Math.min(pendingLeaves * 35, 100)}%` }} />
          </div>
          <p className="text-[10px] text-slate-500 mt-1">Review Required</p>
        </div>

        {/* Card 8: Substitutions */}
        <div className="card p-4 stat-card-elevate animate-slide-up-fade stagger-8 border-slate-800 bg-slate-900/90 relative overflow-hidden">
          ...
          <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden mt-2">
            <div className="h-full bar-fill-anim bg-teal-400" style={{ width: `${Math.min(substitutionsToday * 25, 100)}%` }} />
          </div>
          ...
          <div className="card p-6 bg-slate-900/90 border border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-bold text-white flex items-center gap-2">
                <svg className="w-5 h-5 text-amber-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              <div className="p-4 rounded-xl bg-slate-800/80 border border-slate-700/60 stat-card-elevate">
              ...
              <div className="p-4 rounded-xl bg-slate-800/80 border border-slate-700/60 stat-card-elevate">
                ...
                <p className="font-bold text-amber-300 mt-1 text-sm">{sectionsCount} Parallel Sections</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
              <button onClick={() => onNavigate("faculty")} className="p-3 rounded-xl bg-slate-800/60 hover:bg-slate-800 border border-slate-700/80 text-left transition-all text-xs font-semibold text-slate-200 flex items-center gap-2 hover:border-amber-500/40">
                <svg className="w-4 h-4 text-amber-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                Faculty Directory
              </button>
              <button onClick={() => onNavigate("leave")} className="p-3 rounded-xl bg-slate-800/60 hover:bg-slate-800 border border-slate-700/80 text-left transition-all text-xs font-semibold text-slate-200 flex items-center gap-2 hover:border-amber-500/40">
                <svg className="w-4 h-4 text-amber-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
                Approve Leaves
              </button>
              <button onClick={() => onNavigate("substitutions")} className="p-3 rounded-xl bg-slate-800/60 hover:bg-slate-800 border border-slate-700/80 text-left transition-all text-xs font-semibold text-slate-200 flex items-center gap-2 hover:border-amber-500/40">
                <svg className="w-4 h-4 text-amber-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>
                Substitutions
              </button>
              <button onClick={() => onNavigate("reports")} className="p-3 rounded-xl bg-slate-800/60 hover:bg-slate-800 border border-slate-700/80 text-left transition-all text-xs font-semibold text-slate-200 flex items-center gap-2 hover:border-amber-500/40">
                <svg className="w-4 h-4 text-amber-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
                Export Reports
              </button>
            </div>
