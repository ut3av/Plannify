...
export default function FacultyAnalyticsModule({ initialFacultyId, onBackToSystem }) {
  ...
  const handleExportExcel = () => {
    exportAnalyticsToExcel(dashboardData, directoryData, rangeKey);
  };

  const handleExportPdf = () => {
    exportAnalyticsToPdf(dashboardData, directoryData, rangeKey);
  };
      ...
      <div className="card p-6 bg-slate-900 border border-slate-800 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl bg-rose-500/15 hover:bg-rose-500/25 border border-rose-500/40 text-xs font-bold text-rose-300 transition-all flex items-center gap-1.5 shadow-sm"
            title="Download NAAC-compliant PDF Analytics Report"
          >
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/40 text-xs font-bold text-emerald-300 transition-all flex items-center gap-1.5 shadow-sm"
            title="Download Faculty Workload Excel (.xlsx) Sheet"
          >
